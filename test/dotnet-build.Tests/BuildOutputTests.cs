...
//        [InlineData("net461", true, true)]
        ...
        public void UnresolvedReferenceCausesBuildToFailAndNotProduceOutput()
        {
            var testInstance = TestAssetsManager.CreateTestInstance("TestProjectWithUnresolvedDependency")
                                                .WithLockFiles();
            var cmd = new BuildCommand(testInstance.TestRoot);
            var result = cmd.Execute();
            result.Should().Fail();

            result.StdOut.Should().Contain("The dependency ThisIsNotARealDependencyAndIfSomeoneGoesAndAddsAProjectWithThisNameIWillFindThemAndPunishThem could not be resolved.");
